        ...
        trip.participants.map(async (participant) => {
          ...
          const message = await mail.sendMail({
            from: {
              ...
              address: "equipe.plann.er@gmail.com",
            },
            to: participant.email,
            subject: `Confirme sua presença na viagem para ${trip.destination} em ${formattedStartsDate}`,
            html: `
            <div style="font-family: sans-serif; font-size: 16px; line-height: 1.6">
              <p>
                Você foi convidado para participar de uma viagem para
                <strong> ${trip.destination} </strong> nas datas de 
                <strong>${formattedStartsDate}</strong> até <strong>${formattedEndsDate}</strong>
              </p>
              <p></p>
